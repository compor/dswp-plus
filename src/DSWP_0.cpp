...
bool QNode::operator<(const QNode &y) const { return latency < y.latency; }
...
DSWP::DSWP() : LoopPass(ID) { loopCounter = 0; }
...
bool DSWP::doInitialization(Loop *L, LPPassManager &LPM) {
  ...
  if (produce == NULL) { // the first time, we need to link them
    ...
    Type *void_ty = Type::getVoidTy(ctx), *int32_ty = Type::getInt32Ty(ctx),
         *int64_ty = Type::getInt64Ty(ctx),
         *int8_ptr_ty = Type::getInt8PtrTy(ctx), *elTy = int64_ty;
    ...
    produce = Function::Create(produce_ft, Function::ExternalLinkage,
                               "sync_produce", mod);
    ...
    Function *consume = Function::Create(consume_ft, Function::ExternalLinkage,
                                         "sync_consume", mod);
    ...
    Function *join =
        Function::Create(join_ft, Function::ExternalLinkage, "sync_join", mod);
    join->setCallingConv(CallingConv::C);

    // add sync_init
    FunctionType *init_ft = FunctionType::get(void_ty, false);
    Function *init =
        Function::Create(init_ft, Function::ExternalLinkage, "sync_init", mod);
    ...
    Function *delegate = Function::Create(
        delegate_ft, Function::ExternalLinkage, "sync_delegate", mod);
    ...
    Function *show =
        Function::Create(show_ft, Function::ExternalLinkage, "showValue", mod);
    ...
    Function *show2 =
        Function::Create(show2_ft, Function::ExternalLinkage, "showPlace", mod);
    ...
    Function *show3 =
        Function::Create(show3_ft, Function::ExternalLinkage, "showPtr", mod);
...
bool DSWP::runOnLoop(Loop *L, LPPassManager &LPM) {
  ...
  if (generated.find(L->getHeader()->getParent()) !=
      generated.end()) // this is the generated code
    return false;
