...
void DSWP::preLoopSplit(Loop *L) {
  ...
  argStructTy = StructType::create(*context, argTypes,
                                   "argstruct_" + itoa(loopCounter) + "_ty");
  ...
  CastInst *argStruct_voidPtr = CastInst::CreatePointerCast(
      argStruct, int8_ptr_t, "argstruct_" + itoa(loopCounter) + "_cast",
      brInst);
  ...
  if (!liveout.empty()) {
    ...
    for (unsigned int i = 0; i < liveout.size(); i++) {
      ...
      // load the out value
      LoadInst *outVal =
          new LoadInst(ele_addr, liveout[i]->getName() + "_load", brInst);

      replacement_map[liveout[i]] = outVal;
    }

    // replace any uses *outside of the loop* with this load instruction
    for (Function::iterator bi = m_func->begin(), be = m_func->end(); bi != be;
         ++bi) {
      BasicBlock &bb = *bi;
      if (!L->contains(&bb)) {
  ...
  for (int i = 0; i < MAX_THREAD; i++) {
    ...
    if (relbb.empty()) {
      ...
      ReturnInst *newRet = ReturnInst::Create(
          *context, Constant::getNullValue(Type::getInt8PtrTy(*context)),
          newBody);
      return;
    }
    ...
    for (set<BasicBlock *>::iterator bi = relbb.begin(), be = relbb.end();
      ...
      BBMap[BB] = BasicBlock::Create(
          *context, BB->getName().str() + "_" + itoa(i), curFunc, newExit);
    }
    ...
    for (set<BasicBlock *>::iterator bi = relbb.begin(), be = relbb.end();
      ...
      for (BasicBlock::iterator ii = BB->begin(), ie = BB->end(); ii != ie;
        ...
        if (TerminatorInst *newT = dyn_cast<TerminatorInst>(newInst)) {
          ...
          for (unsigned int j = 0, je = phi->getNumIncomingValues(); j < je;
               j++) {
    ...
    BitCastInst *castArgs =
        new BitCastInst(args, PointerType::get(argStructTy, 0), "args");
    ...
    for (unsigned int j = 0, je = livein.size(); j < je; j++) {
      ...
      //
      castArgs->dump();
      cast<PointerType>(castArgs->getType()->getScalarType())
          ->getElementType()
          ->dump();
      ...
      GetElementPtrInst *ele_addr =
          GetElementPtrInst::Create(nullptr, castArgs, gep_args,
                                    livein[j]->getName() + "_arg", newToHeader);
    ...
    for (unsigned int j = 0; j < liveout.size(); j++) {
      if (getInstAssigned(liveout[j]) == i) {
        ...
        GetElementPtrInst *ele_addr = GetElementPtrInst::CreateInBounds(
            out_addr, gep_args, liveout[j]->getName() + "_outptr", newRet);

        // save it
        StoreInst *store =
            new StoreInst(instMap[i][liveout[j]], ele_addr, newRet);
      }
    }
  }
}
