...
void DSWP::buildPDG(Loop *L) {
  ...
  for (Loop::block_iterator bi = L->getBlocks().begin();
    ...
    for (BasicBlock::iterator ii = BB->begin(); ii != BB->end(); ii++) {
      ...
      for (Value::use_iterator ui = ii->use_begin(); ui != ii->use_end();
           ui++) {
      ...
      if (mdr.isNonLocal()) {
        if (CallInst *call = dyn_cast<CallInst>(inst)) {
          ...
          for (res_t::iterator ri = res.begin(), re = res.end(); ri != re;
               ++ri) {
  ...
  FunctionType *functype =
      FunctionType::get(Type::getVoidTy(*curcontext), false);
  ...
  Constant *cfunc = newmodule.getOrInsertFunction(
      "dummyloopunroll", Type::getVoidTy(getGlobalContext()), int_arg, NULL);
  ...
  */
  ...
  for (std::vector<BasicBlock *>::iterator it = bb_ordered.begin();
       it != bb_ordered.end(); ++it) {
    cout << (*it)->getName().str() << ", ";
  }
  cout << endl;
  ...
  BasicBlock *dummyexitblock =
      BasicBlock::Create(ctxt, "exitblock", ctrlfunc, 0);
  ReturnInst::Create(ctxt, 0, dummyexitblock);

  cout << ">>Printing out names of dummy blocks inside our fake function"
       << endl;
  ...
  for (SmallVector<BasicBlock *, 10>::iterator it = bb_exits.begin(),
       ...
       it != ie; ++it) {
    BasicBlock *bbend = *it;
    for (pred_iterator pi = pred_begin(bbend), pe = pred_end(bbend); pi != pe;
         ++pi) {
      if (L->contains(*pi)) // block in loop?
        if (returnblocks.find(*pi) == returnblocks.end()) // to insert?
        {
          cout << ">>Adding " << (*pi)->getName().str() << " as exiting block"
               << endl;
  ...
  if (!bb_ordered.empty()) {
    ...
    do {
      ...
      if (returnblocks.find(*it) != returnblocks.end()) {
        SI1 = builder1.CreateSwitch(
            ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0),
            dummyexitblock, nsucc + 1);
        SI2 = builder2.CreateSwitch(
            ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0),
            dummyexitblock, nsucc + 1);

        SI1->addCase(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0),
                     dummyexitblock);
        SI2->addCase(ConstantInt::get(Type::getInt64Ty(getGlobalContext()), 0),
                     dummyexitblock);
      ...
      for (unsigned i = 0; i < nsucc; i++) {
        ...
        if (L->contains(bsucc)) { // successor is still inside loop?
          ...
          SI1->addCase(
              ConstantInt::get(Type::getInt64Ty(getGlobalContext()), i + 1),
              destblock1);
